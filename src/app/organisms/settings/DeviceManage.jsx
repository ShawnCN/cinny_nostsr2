...
function DeviceManage() {
  ...
  const handleRename = async (device) => {
    ...
    try {
      await mx.setDeviceDetails(device.deviceId, {
  ...
  const handleRemove = async (device) => {
    ...
    await authRequest(`Logout "${device.display_name}"`, async (auth) => {
      await mx.deleteDevice(device.deviceId, auth);
    });
  ...
  deviceList
    ...
    .forEach((device) => {
      const isVerified = isCrossVerified(device.deviceId);
      if (isVerified === true) {
